...
# 判断是否符合系统迹象
#
def is_need_to_alert(df: pd.DataFrame, extrema):
    bar_len = df.__len__()
    # 1. 判断最近90根K有6个以上端点
    point = extrema[-6] if len(extrema) >= 6 else None
    if point is None or bar_len - point['index'] > 90:
        print("震荡区间不满足")
    ...
    high_lines, key_high_points = is_difference_less_than_threshold(highs[-12:], threshold = atr[-1]/2)
    low_lines, key_low_points = is_difference_less_than_threshold(lows[-12:], threshold = atr[-1]/2)
