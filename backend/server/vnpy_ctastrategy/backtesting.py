# 引入本地包
from victor.service_client.redis_client import redis_client_cls
...
from vnpy.trader.utility import round_to, extract_vt_symbol, transfer_minute_bars_to_window_bars
...
class BacktestingEngine:
    ...
    def __init__(self) -> None:
        """"""
        self.database_init_args: tuple = None
        self.vt_symbol: str = ""
        self.symbol: str = ""
    ...
    def set_parameters(
        ...
        """"""
        self.database_init_args = database_init_args
        self.mode = mode
    ...
    def load_data(self) -> None:
        """"""
        # victor 先从redis中获取数据，如果有数据就不去数据库读
        redis_key = f"vnpy:{self.symbol}:{self.exchange.value}:{self.interval.value}:{self.start.strftime('%Y%m%d')}:{self.end.strftime('%Y%m%d')}"
        restored_data = redis_client_cls.get_data_from_redis(redis_key)
        if restored_data is not None:
            # print('=======get_data_from_redis=====', restored_data)
            self.history_data = restored_data
            self.history_hour_data = transfer_minute_bars_to_window_bars(self.history_data, 60)
        ...
        # victor
        redis_client_cls.set_data_to_redis_expire(redis_key, self.history_data, 60 * 60 * 24 * 365)
        self.history_hour_data = transfer_minute_bars_to_window_bars(self.history_data, 60)
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True) -> dict:
        ...
        if positive_balance:
            ...
            if return_std:
                ...
                log_risk_free = np.log(1 + daily_risk_free) # 将无风险利率转换为对数形式
                df['excess_return'] = df['return'] - log_risk_free
                # 找到所有负的超额回报
                negative_excess_returns = df['excess_return'][df['excess_return'] < 0]
                # 计算下行标准差
                downside_std = np.std(negative_excess_returns, ddof=1)
                # 计算年化的平均超额回报（对数形式）
                annual_excess_return = df['excess_return'].mean() * self.annual_days

                # 计算索提诺比率
                sortino_ratio = annual_excess_return / (downside_std * np.sqrt(self.annual_days))

                sharpe_ratio: float = annual_excess_return / return_std * np.sqrt(self.annual_days)
            else:
                sharpe_ratio: float = 0
    ...
    def show_chart(self, df: DataFrame = None, statistics: dict = None) -> None:
        """"""
        # Check DataFrame input exterior
        if df is None:
            df: DataFrame = self.daily_df
        ...
        fig = make_subplots(
            rows=5,
            cols=1,
            subplot_titles=["Balance", "Drawdown", "Daily Pnl", "Pnl Distribution", "table"],
            vertical_spacing=0.06,
            row_heights=[0.1, 0.1, 0.1, 0.1, 0.6],  # 分配相等的高度给所有subplots，或根据需要调整
            specs=[[{"type": "xy"}],  # 第一个subplot是标准xy类型
           [{"type": "xy"}],  # 第二个也是
           [{"type": "xy"}],  # ...
           [{"type": "xy"}],
           [{"type": "table"}]]  # 第五个是专门用于表格的subplot
        )
        ...
        if statistics is not None:
            header_values = ['指标', '数据值']

            cell_values = [
                    ["首个交易日：", statistics["start_date"]],
                    ["最后交易日：", statistics["end_date"]],

                    ["总交易日：", statistics["total_days"]],
                    ["盈利交易日：", statistics["profit_days"]],
                    ["亏损交易日：", statistics["loss_days"]],

                    ["起始资金：", f"{self.capital:.2f}"],
                    ["结束资金：", f'{statistics["end_balance"]:.2f}'],

                    ["总收益率：", f'{statistics["total_return"]:.2f}%'],
                    ["年化收益：", f'{statistics["annual_return"]:.2f}%'],
                    ["最大回撤", f'{statistics["max_drawdown"]:.2f}'],
                    ["百分比最大回撤：", f'{statistics["max_ddpercent"]:.2f}%'],
                    ["最长回撤天数：", statistics["max_drawdown_duration"]],

                    ["总盈亏：", f'{statistics["total_net_pnl"]:.2f}'],
                    ["总手续费：", f'{statistics["total_commission"]:.2f}'],
                    ["总滑点：", f'{statistics["total_slippage"]:.2f}'],
                    ["总成交金额：", f'{statistics["total_turnover"]:.2f}'],
                    ["总成交笔数：", f'{statistics["total_trade_count"]:.2f}'],

                    ["日均盈亏：", f'{statistics["daily_net_pnl"]:.2f}'],
                    ["日均手续费：", f'{statistics["daily_commission"]:.2f}'],
                    ["日均滑点：", f'{statistics["daily_slippage"]:.2f}'],
                    ["日均成交金额：", f'{statistics["daily_turnover"]:.2f}'],
                    ["日均成交笔数：", statistics["daily_trade_count"]],

                    ["日均收益率：", f'{statistics["daily_return"]:.2f}%'],
                    ["收益标准差：", f'{statistics["return_std"]:.2f}%'],
                    ["夏普比率：", f'{statistics["sharpe_ratio"]:.2f}'],
                    ["索提诺比率：", f'{statistics["sortino_ratio"]:.2f}'],
                    ["收益回撤比：", f'{statistics["return_drawdown_ratio"]:.2f}'],
                ]
            # 转置 cell_values 以便按行显示
            cell_values_transposed = list(map(list, zip(*cell_values)))

            # 创建表格
            table_scatter = go.Table(
                header=dict(
        ...
        fig.add_trace(table_scatter, row=5, col=1)
    ...
    def cross_limit_order(self) -> None:
        ...
        for order in list(self.active_limit_orders.values()):
            ...
            print('触发limit订单', self.bar, order, long_best_price, short_best_price)
            print('触发limit订单', trade_price, self.datetime)
    ...
    def cross_stop_order(self) -> None:
        ...
        for stop_order in list(self.active_stop_orders.values()):
            ...
            print('触发stop订单', self.bar, stop_order, long_best_price, short_best_price)
            print('触发stop订单', trade_price, self.datetime)
            trade: TradeData = TradeData(
                symbol=order.symbol,
                exchange=order.exchange,
                orderid=order.orderid,
...
class DailyResult:
    ...
    def calculate_pnl(
        ...
        for trade in self.trades:
            ...
            # TODO:打印日均变化,待删除
            # print('交易变化 trade', trade.__dict__)
            # print('交易变化 self.end_pos', self.end_pos)
            # print('交易变化 当前turnover', turnover)
            # print('交易变化 self.trading_pnl', self.trading_pnl)
            # print('交易变化 self.slippage', self.slippage)
            # print('交易变化 self.turnover', self.turnover)
            # print('交易变化 self.commission', self.commission)

        # Net pnl takes account of commission and slippage cost
        self.total_pnl = self.trading_pnl + self.holding_pnl
        self.net_pnl = self.total_pnl - self.commission - self.slippage
        # print('交易变化 self.holding_pnl', self.holding_pnl)
        # print('交易变化 self.total_pnl', self.total_pnl)
        # print('交易变化 self.net_pnl', self.net_pnl)
...
def load_bar_data(
    ...
    """"""
    database: BaseDatabase = get_database(database_init_args)
