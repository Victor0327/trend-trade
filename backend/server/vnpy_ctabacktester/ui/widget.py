...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        self.hour_candle_button: QtWidgets.QPushButton = QtWidgets.QPushButton("小时图表")
        self.hour_candle_button.clicked.connect(self.show_hour_candle_chart)
        self.hour_candle_button.setEnabled(False)
        ...
        result_grid.addWidget(self.hour_candle_button, 2, 0)
        ...
        self.hour_candle_dialog: CandleChartDialog = CandleChartDialog()
    ...
    def process_backtesting_finished_event(self, event: Event) -> None:
        ...
        if interval != Interval.TICK.value:
            ...
            self.hour_candle_button.setEnabled(True)
    ...
    def start_backtesting(self) -> None:
        ...
        if result:
            ...
            self.hour_candle_dialog.clear_data()
    ...
    def show_hour_candle_chart(self) -> None:
        """"""
        if not self.hour_candle_dialog.is_updated():
            history: list = self.backtester_engine.get_history_hour_data()
            self.hour_candle_dialog.update_history(history)

            trades: List[TradeData] = self.backtester_engine.get_all_trades()
            self.hour_candle_dialog.update_trades(trades, dt_hour=True)

        self.hour_candle_dialog.exec_()
...
class CandleChartDialog(QtWidgets.QDialog):
    ...
    def update_trades(self, trades: list, dt_hour: bool = False) -> None:
        """"""
        ...
        for d in trade_pairs:
            if dt_hour is False:
                ...
                open_dt = d["open_dt"]
                open_dt_hour = datetime(open_dt.year, open_dt.month, open_dt.day, open_dt.hour, tzinfo=open_dt.tzinfo)
                open_ix = self.dt_ix_map[open_dt_hour]
                close_dt = d["close_dt"]
                close_dt_hour = datetime(close_dt.year, close_dt.month, close_dt.day, close_dt.hour, tzinfo=close_dt.tzinfo)
                close_ix = self.dt_ix_map[close_dt_hour]
