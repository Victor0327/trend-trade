...
def fetch_data(symbol):
  ...
  url = f"https://vip.stock.finance.sina.com.cn/forex/api/jsonp.php/var%20_fx_s{symbol}_{interval}=/NewForexService.getDayKLine?symbol=fx_s{symbol}&scale={interval}"
  ...
  if match:
      ...
      days_data = json_str.split('|')
      tuple_list = [tuple(day.split(',')) for day in days_data]
      # d、o、l、h、c
      # 先低再高
      # print(tuple_list)
      ...
      # 只取当前小时和上一个小时的数据
      filtered_tuple_list = [x for x in tuple_list if current_day in tuple_list[0] or last_day in tuple_list[0]]
      ...
      logging.info(filtered_tuple_list)

      # insert_data_sql = get_insert_sql(params, tuple_list)
      # print(insert_data_sql)
      # post_db.run_sql_to_commit(insert_data_sql)

      if filtered_tuple_list.__len__() > 0:
        insert_data_sql = get_insert_sql(params, filtered_tuple_list)
        post_db.run_sql_to_commit(insert_data_sql)
  else:
