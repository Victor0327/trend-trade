...
def fetch_data(symbol):
  ...
  url = f"https://stock2.finance.sina.com.cn/futures/api/jsonp.php/var%20_{symbol}_{interval}=/GlobalFuturesService.getGlobalFuturesDailyKLine?symbol={symbol}"
  ...
  if match:
      ...
      # 只取今天和昨天的数据
      filtered_data_list = [x for x in data_list if current_day in x['date'] or last_day in x['date']]
