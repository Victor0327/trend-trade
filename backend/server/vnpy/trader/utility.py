...
# 我自己加的辅助函数
def transfer_minute_bars_to_window_bars(bar_list: list[BarData], window) -> list:
    """
    将分钟K线一次性合成为大周期K线
    """
    if not bar_list:
    ...
    if window == 1:
        return bar_list.copy()

    new_bar_list: list[BarData] = []
    for i in range(0, len(bar_list), window):
         # 创建新的 BarData 对象作为窗口条
        window_bar = BarData(
            open_price=bar_list[i].open_price,
            high_price=bar_list[i].high_price,
            low_price=bar_list[i].low_price,
            close_price=bar_list[i].close_price,
            volume=0,
            turnover=0,
            open_interest=bar_list[i].open_interest,
            interval=None,
            gateway_name=bar_list[i].gateway_name,
            symbol=bar_list[i].symbol,
            exchange=bar_list[i].exchange,
            datetime=bar_list[i].datetime,
        )

        for j in range(window):
            if i + j < len(bar_list):  # 确保不超出列表范围
                bar: BarData = bar_list[i + j]
        ...
        new_bar_list.append(window_bar)
    return new_bar_list